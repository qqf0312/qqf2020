...
impl AccountEntry {
    ...
    //qqf-code
    pub fn insert_account_storage_changes(&mut self, k: H256, v: H256){
        let mut a = self.account().unwrap().clone();
        a.set_storage(k.clone(), v.clone());
        self.account = Some(a);
    }
    pub fn change_nonce(&mut self, u :U256){
        let mut a = self.account().unwrap().clone();
        if a.nonce() < &u {
            a.change_nonce(u) ;
        }
    ...
    pub fn print_account(&self){
        let a = self.account().unwrap().clone();
        a.print_account();
    }
    pub fn insert_account_storage_cache(&mut self, k: H256, v:H256){
        let mut a = self.account().unwrap().clone();
        a.storage_cache_insert(k.clone(), v.clone());
        self.account = Some(a);
    }
    //qqf-code-end
}
...
pub struct State<B: Backend> {
    ...
    //一致性读集
    pub consist_read_set: HashMap<H256, H256>,
    //临时存放写集区域
    pub _temp_storage_change: HashMap<H256, H256>,
}
...
impl<B: Backend> State<B> {
    ...
    pub fn new(mut db: B, account_start_nonce: U256, factories: Factories) -> State<B> {
        ...
        State {
            ...
            //初始化一致性读集
            consist_read_set: HashMap::new(),
            //临时存放写集区域
            _temp_storage_change: HashMap::new(),
        }
    }
    ...
    //qqf-code
    pub fn clone_cache(&self)-> HashMap<Address, AccountEntry>{
        let x =self.cache();
        let iter = x.iter();
    ...
    pub fn insert_consist_read_set(&mut self, consist_read_set: HashMap<H256, H256>){
        self.consist_read_set = consist_read_set;
    }
    pub fn exchange_cache(&mut self, real_cache: HashMap<Address, AccountEntry>){
        let x = RefCell::new(real_cache);
        self.cache = x;
    }
    //qqf-code-end 
    ...
    pub fn storage_at(&self, address: &Address, key: &H256) -> trie::Result<H256> {
...

        {
            ...
            //info!("寻找key:{:?}",&key);
            /*
            //storage_at_mut中会读读写集
            if !self.consist_read_set.is_empty(){
                let temp_h256 = self.consist_read_set.get(key);
                info!("寻找key:{:?}",&key);
                if temp_h256 != None{
                    let value = temp_h256.unwrap().clone();
                    info!("在一致性依赖中找到所需要的数据:{:?}",&value);
                    //读一次数据就删除，防止读脏数据
                    //self.consist_read_set.remove(key);
                    //let x = Ok(value)
                    return Ok(value);
                }
            }
            info!("一致性读集合中找不到需要的数据");
            */
    ...
    //qqf-code
    pub fn storage_at_mut(&mut self, address: &Address, key: &H256) -> trie::Result<H256> {
        // Storage key search and update works like this:
        // 1. If there's an entry for the account in the local cache check for the key and return it if found.
        // 2. If there's an entry for the account in the global cache check for the key or load it into that account.
        // 3. If account is missing in the global cache load it into the local cache and cache the key there.
            ...
            */
            if !self.consist_read_set.is_empty(){
                ...
                if temp_h256 != None{
                    ...
                    info!("删除所读数据在一致性读位置");
                    //let x = Ok(value)
                    let mut local_cache = self.cache.borrow_mut();
                    let mut local_account = local_cache.get(&address.clone()).unwrap().clone();
                    info!("获得地址{:?}的account",&address);
                    local_account.insert_account_storage_cache(key.clone(), value.clone());
                    //info!("插入local_cache{:?}",&local_account);
                    local_cache.insert(address.clone(), local_account);
                    return Ok(value);
                }
            }
            //info!("一致性读集合中找不到需要的数据{:?}",&key);
            ...
            if let Some(maybe_acc) = local_cache.get(address) {
                match maybe_acc.account {
                    Some(ref account) => {
                        if let Some(value) = account.cached_storage_at(key) {
                            //info!("在内存storage_change中或者storage_cache中找到所需数据");
                            return Ok(value);
                        } else {
                            local_account = Some(maybe_acc);
                        }
    ...
    pub fn deocc_storage_at(&self, address: &Address, key: &H256) -> trie::Result<H256> {
        // Storage key search and update works like this:
        // 1. If there's an entry for the account in the local cache check for the key and return it if found.
        // 2. If there's an entry for the account in the global cache check for the key or load it into that account.
        // 3. If account is missing in the global cache load it into the local cache and cache the key there.
    ...
    pub fn set_temp_storage_change(&mut self, key: H256, value: H256){
        self._temp_storage_change.insert(key.clone(), value.clone());
    }
