...
impl Account {
    ...
    pub fn print_account(&self){
        info!("{:?}",self.balance());
        info!("{:?}",self.nonce());
        info!("{:?}",self.storage_root());
        info!("{:?}",self.storage_cache());
        info!("{:?}",self.storage_changes());
        info!("{:?}",self.code_hash());
        info!("{:?}",self.code_size());
        info!("{:?}",self.code_cache);
        info!("{:?}",self.code_filth);
        info!("{:?}",self.abi_hash());
        info!("{:?}",self.abi_size());
        info!("{:?}",self.abi_cache);
        info!("{:?}",self.abi_filth);
        info!("{:?}",self.get_address_hash());
    }
    /// General constructor.
    ...
    //qqf-code
    pub fn storage_changes_get(&self) -> BTreeMap<String, String>{
        let mut result = BTreeMap::new();
        let string = self.get_address_hash();
        for (k, v) in self.storage_changes().iter() {
            info!("k(H256):{:?}",&k);
            info!("v(H256):{:?}",&v);
            let key = String::from("0x") + &string.clone() + &hex::encode(*k);
            let value = String::from("0x") + &string.clone() + &hex::encode(*v);
            
            result.insert(key.clone(), value.clone());
        }
        result
    }
    pub fn change_nonce(&mut self, x: U256){
        self.nonce = x ;
    }
    pub fn storage_cache_insert(&mut self, key: H256, value: H256){
        self.storage_cache.borrow_mut().insert(key.clone(), value.clone());
        info!("插入storage_cache:k:{:?},v:{:?}",&key, &value);
    }
    //qqf-code-end
