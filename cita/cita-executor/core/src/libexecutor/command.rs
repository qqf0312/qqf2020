...
impl Commander for Executor {
    ...
    fn clone_executor_reader(&mut self) -> Self {
        ...
        //qqf-code
        let fake_fsm_req_receiver = self.fake_fsm_req_receiver.clone();
        let fake_fsm_resp_sender = self.fake_fsm_resp_sender.clone();
        let tdg_req_receiver = self.tdg_req_receiver.clone();
        let tdg_resp_sender = self.tdg_resp_sender.clone();
        //qqf-code-end
        Executor {
