...
use super::fsm::FSM;
use super::_transaction::{BincodeGraph};
...
pub use super::batching_occ::BatchingOcc;
use crate::libexecutor::_transaction::TransactionInfo;
...
pub struct Executor {
    ...
    //qqf-code
    pub fake_fsm_req_receiver: Receiver<OpenBlock>,
    pub fake_fsm_resp_sender: Sender<Vec<u8>>,
    pub tdg_req_receiver: Receiver<Vec<u8>>,
    pub tdg_resp_sender: Sender<bool>,
    pub raw_bytes_tdg: Vec<u8>,
    pub is_prime: bool,
...
#[derive(Clone)]
pub struct RwSet{
    pub read_set : BTreeMap<usize,BTreeMap<String,String>>,
    pub write_set : BTreeMap<usize,BTreeMap<String,String>>,
}
impl RwSet{
    pub fn new()->Self{
        let x = RwSet{
    ...
    pub fn insert_read_set(&mut self, key: usize, value: BTreeMap<String, String>){
        self.read_set.insert(key, value);
    }
    pub fn insert_write_set(&mut self, key: usize, value: BTreeMap<String, String>){
...
impl Executor {
    ...
    pub fn init(
        ...
        //qqf-code
        fake_fsm_req_receiver: Receiver<OpenBlock>,
        fake_fsm_resp_sender: Sender<Vec<u8>>,
        tdg_req_receiver: Receiver<Vec<u8>>,
        tdg_resp_sender: Sender<bool>,
        //qqf-code-end
    ) -> Executor {
        ...
        let mut executor = Executor {
            ...
            eth_compatibility,
            //qqf-code
            fake_fsm_req_receiver,
            fake_fsm_resp_sender,
            tdg_req_receiver,
            tdg_resp_sender,
        ...
        loop {
            match self.recv() {
                ...
                (None, Some(block), None, None) => {
                    //换成real_into_fsm
                    info!("收到openblock");
                    info!("执行前，检查tdg图：{:?}",&self.tdg);
                    //qqf-code
                    //串行执行函数fn test_deocc_fsm_execute
                    self.test_deocc_fsm_execute(block.clone(), 0);
                    
                    let fsm_resp = self.test_1_deocc_fsm_execute(self.to_executed_block(block.clone()), block.clone(), 0);
                    //let fsm_resp = self.test_deocc_fsm_execute(self.to_executed_block(block.clone()), 0);
                    
                    //let fsm_resp = self.into_fsm(block);
                    //self.test_1_deocc_fsm_execute(self.to_executed_block(block.clone()), 0);
                    //info!("into_fsm:{:?}",&fsm_resp);
                    //qqf-code-end
                    //let fsm_resp = self.into_fsm(block);
                    //info!("回放后：{:?}",&fsm_resp);
                    self.fsm_resp_sender.send(fsm_resp);
                ...
                (None, None, Some(block), None) => {
                    info!("收到fake msg block:{:?}-th",block.number().clone());
                    //获得预执行过后的函数依赖图tdg
                    let bocc = self.pre_into_fsm(block);
                    //打印出tdg节点的transactioninfo相关信息
                    bocc.get_tdg_transactioninfo();
                    //向postman发送bocc
                    //self.fake_fsm_resp_sender.send(bocc.clone());
                    //转换图成为可以用bincode编译的结构体：bincodegraph
                    let (vec_transactioninfo, vec_usize) = bocc.get_graph_in_vec();
                    let toposort = bocc.tdg_toposort.clone();
                    let bincodegraph = BincodeGraph::new(vec_transactioninfo, vec_usize, toposort.clone());
                    self.tdg = bincodegraph.create_graph().clone();
                    self.tdg_toposort = toposort;
                    //info!("预执行得到的依赖图:{:?}",&self.tdg);
                    let target = Some(bincodegraph);
                    let limit = Bounded(18446744073709551615);
                    //序列化
                    let _encoded: Vec<u8> = serialize(&target, limit).unwrap();
                    self.fake_fsm_resp_sender.send(_encoded);
                    /*
                    //反序列化
                    //info!("开始反序列化");
                    let decoded: Option<BincodeGraph> = deserialize(&encoded[..]).unwrap();
                    if decoded != None{
                        //info!("反序列化不为空");
                ...
                (None, None, None, Some(tdg)) => {
                    self.raw_bytes_tdg = tdg ;
                    //info!("开始反序列化");
                    if self.raw_bytes_tdg.is_empty(){
                        self.tdg_resp_sender.send(false);
                        continue;
                    }
                    let decoded: Option<BincodeGraph> = deserialize(&self.raw_bytes_tdg.clone()[..]).unwrap();
                    if decoded != None{
                        //info!("反序列化不为空");
                        let x = decoded.unwrap().clone();
                        let g = x.create_graph();
                        self.tdg = g.clone();
                        self.tdg_toposort = x.toposort.clone();
                        //let iter = x.nodes_set;
                        //info!("tdg_graph:{:?}",self.tdg.clone());
                    }
                    //info!("反序列化结束");
                    self.tdg_resp_sender.send(true);
                    info!("收到tdg，发送回应给postman！");
                }
            }
        }
    }
    fn recv(&self) -> (Option<Command>, Option<OpenBlock>, Option<OpenBlock>, Option<Vec<u8>>) {
        let err_flag = (None, None, None, None);
        select! {
            recv(self.command_req_receiver, command_req) => {
                match command_req {
                    Some(command_req) => (Some(command_req), None, None, None),
                    None => err_flag,
                }
            },
            recv(self.fsm_req_receiver, fsm_req) => {
                match fsm_req {
                    Some(fsm_req) => (None, Some(fsm_req), None, None),
                    None => err_flag,
                }
            },
            recv(self.fake_fsm_req_receiver, fake_fsm_req) => {
                match fake_fsm_req{
                    Some(fake_fsm_req) => (None, None, Some(fake_fsm_req), None),
                    None => err_flag,
                }
            },
            recv(self.tdg_req_receiver, tdg_req) => {
                match tdg_req{
                    Some(tdg_req) => (None, None, None, Some(tdg_req)),
                    None => err_flag,
                }
            }
        }
