...
use std::sync::Arc;
pub use crate::state::AccountEntry;
...
impl ExecutedBlock {
    ...
    //qqf-code
    pub fn exchange_state_cache(&mut self, real_cache: HashMap<Address, AccountEntry>){
        self.state.exchange_cache(real_cache);
    }
    //qqf-code end
    ...
    pub fn close(mut self, conf: &BlockSysConfig) -> ClosedBlock {
        ...
        //receipts给爷爬
        /*
        let l = self.receipts.clone().len();
        if l != 0{
            let mut receipt_vec = Vec::new();
            let fake_receipt = Receipt::default();
            receipt_vec.push(fake_receipt.clone());
            //receipt_vec.push(self.receipts[l-1].clone());
            self.receipts = receipt_vec;
        }
        */
