    ...
    B: StateBackend,
{
    fn storage_at(&self, key: &H256) -> evm::Result<H256> {
        //info!("from externalities");
        self.state
            .storage_at(&self.origin_info.address, key)
            .map_err(Into::into)
    }

    fn storage_at_mut(&mut self, key: &H256) -> evm::Result<H256> {
        //info!("from mut externalities");
        self.state
            .storage_at_mut(&mut self.origin_info.address, key)
            .map_err(Into::into)
    }

    fn set_temp_storage_change(&mut self, key: H256, value: H256){
        self.state.set_temp_storage_change(key.clone(), value.clone());
    }
