...
fn main() {
    ...
    //qqf-code
    let (fake_fsm_req_sender, fake_fsm_req_receiver) = crossbeam_channel::unbounded();
    let (fake_fsm_resp_sender,fake_fsm_resp_receiver) = crossbeam_channel::unbounded();
    let (tdg_req_sender,tdg_req_receiver) = crossbeam_channel::unbounded();
    let (tdg_resp_sender,tdg_resp_receiver) = crossbeam_channel::unbounded();
    //qqf-code-end
    start_pubsub(
        "executor",
        routing_key!([
    ...
    loop {
        ...
        let mut executor = Executor::init(
            ...
            //qqf-code
            fake_fsm_req_receiver.clone(),
            fake_fsm_resp_sender.clone(),
            tdg_req_receiver.clone(),
            tdg_resp_sender.clone(),
            //qqf-code-end
        );
        ...
        let mut postman = Postman::new(
            ...
            command_resp_receiver.clone(),
            //qqf-code
            fake_fsm_req_sender.clone(),
            fake_fsm_resp_receiver.clone(),
            tdg_req_sender.clone(),
            tdg_resp_receiver.clone(),
            //qqf-code-end
        );
        postman.do_loop();
