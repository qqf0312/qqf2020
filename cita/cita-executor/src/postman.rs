...
//use crate::core::libexecutor::batching_occ;
use crate::types::ids::BlockId;
//use crate::core::tests::helpers;
use cita_types::{Address, H256, U256};
//use core_executor::libexecutor::batching_occ::Vec<u8>;
...
pub struct Postman {
    ...
    //qqf-code
    fake_fsm_req_sender: Sender<OpenBlock>,
    fake_fsm_resp_receiver: Receiver<Vec<u8>>,
    tdg_req_sender: Sender<Vec<u8>>,
    tdg_resp_receiver: Receiver<bool>,
    //qqf-code-end
...
impl Postman {
    ...
    pub fn do_loop(&mut self) {
        ...
        loop {
            match self.recv() {
                ...
                (Some(_), Some(_), None, None) | (Some(_), None, Some(_), None) | (Some(_), None, None, Some(_)) |
                (None, Some(_), Some(_), None) | (None, None, Some(_), Some(_)) | (None, Some(_), None, Some(_)) |
                (Some(_), Some(_), Some(_), None) | (Some(_), Some(_), None, Some(_)) | (None, Some(_), Some(_), Some(_)) | (Some(_), None, Some(_), Some(_))=> return,
                (Some((key, msg_vec)), None, None, None) => {
                    let result = self.handle_mq_message(key.as_str(), msg_vec);
                    if let Err(rollback_id) = result {
                        self.close(rollback_id);
                        return;
                ...
                (None, Some(closed_block), None, None) => {
                    self.handle_fsm_response(closed_block);
                    self.grow_up();
                    self.execute_next_block();
                    info!("回放结束");
                }
                (None, None, Some(encode), None) => {
                    //传送给共识的tdg图
                    let msg: Message = encode.clone().into();
                    self.response_mq(
                    routing_key!(Executor >> RawBytes).into(),
                    msg.try_into().unwrap(),
                    );
                }
                (None, None, None, Some(_bool)) => {
                    info!("收到传回来的消息，可以执行交易了");
                    self.execute_next_block();
                    //info!("交易回放结束");
                }
            }
    ...
    */
    //qqf-code
    #[cfg_attr(feature = "cargo-clippy", allow(clippy::type_complexity))]
    fn recv(&self) -> (Option<(String, Vec<u8>)>, Option<ClosedBlock>, Option<Vec<u8>>, Option<bool>) {
        select! {
            recv(self.mq_req_receiver, mq_req) => {
                match mq_req {
                    Some(mq_req) => (Some(mq_req), None, None, None),
                    None => (None, None, None, None),
                }
            },
            recv(self.fsm_resp_receiver, fsm_resp) => {
                match fsm_resp {
                    Some(fsm_resp) => (None, Some(fsm_resp), None, None),
                    None => (None, None, None, None),
                }
            },
            recv(self.fake_fsm_resp_receiver, fake_fsm_resp) => {
                match fake_fsm_resp {
                    Some(fake_fsm_resp) => (None, None, Some(fake_fsm_resp), None),
                    None => (None, None, None, None),
                }
            },
            recv(self.tdg_resp_receiver, tdg_resp) => {
                match tdg_resp {
                    Some(tdg_resp) => (None, None, None, Some(tdg_resp)),
                    None => (None, None, None, None),
                }
            }
        }
    ...
    fn handle_mq_message(&mut self, key: &str, msg_vec: Vec<u8>) -> Result<(), BlockId> {
        ...
        match RoutingKey::from(key) {
            ...
            | routing_key!(Chain >> LocalSync) => {
                ...
                let rk = RoutingKey::from(key.clone());
                info!("收到信息:{:?}",key.clone());
                if rk == routing_key!(Consensus >> BlockWithProof){
                    info!("收到BWP信息！");
                    let block_bwp = msg.take_block_with_proof().unwrap();
                    //具体看块的内容
                    info!("bwp:block_conntent:{:?}",block_bwp.get_blk().get_header().get_height().clone());
                    let p = block_bwp.get_proof().clone();
                    //通过into函数能将proof转换成btfproof
                    let btf_p: proof::BftProof = p.clone().into();
                    //判断块的proof是否为空，为空则为预执行bwp。
                    if btf_p.height == MAX {
                        info!("预执行模式启动");
                        //判定是主节点
                        self.is_prime = true;
                        let open_block = OpenBlock::from(block_bwp.get_blk().clone());
                        //info!("预执行:block_conntent:{:?}",block_bwp.number().clone());
                        info!("预执行:open_block:{:?}",open_block.number().clone());
                        //预执行交易块
                        //把openblock发送至fake receive（executor）使其运行预执行操作
                        self.fake_fsm_req_sender.send(open_block.clone());
                        switch = true;
                    }
                    //正式执行bwf
                    else{
                        /*
                        info!("真实执行启动");
                        self.update_backlog(key, msg.clone());
                        info!("update-backlog!!!");
                        self.grow_up();
                        info!("grow-up!!!");
                        //self.tdg_req_sender.send(self.raw_bytes_tdg.clone());
                        self.execute_next_block();
                        info!("execute-next-block!!!");
                        switch = true;
                        */
                        self.update_backlog(key, msg.clone());
                        info!("update-backlog!");
                ...
                else if rk == routing_key!(Consensus >> SignedProposal){
                    self.update_backlog(key, msg.clone());
                    info!("update-backlog!");
                    self.grow_up();
                    info!("grow-up!");
                ...
                //如果为false则代表不是预执行,也不是bwf信息，所以按照原来的流程进行
                if switch == false{
                    //let block_bwp = msg.take_block_with_proof().unwrap();
                    //具体看块的内容
                    //info!("真执行:block_conntent:{:?}",block_bwp.clone());
                    self.update_backlog(key, msg.clone());
                    info!("update-backlog!");
                    self.grow_up();
                    info!("grow-up!");
    ...
    fn update_backlog(&mut self, key: &str, mut msg: Message) -> bool {
        match RoutingKey::from(key) {
            ...
            routing_key!(Consensus >> SignedProposal) => {
                ...
                //qqf-code
                let mut fake_proposal = proposal.take_proposal();
                let mut block = fake_proposal.take_block().clone();
                let mut block_body = block.take_body().clone();
                let repeated_transactions = block_body.take_transactions().clone();
                let mut transactions_vec = repeated_transactions.into_vec();
                info!("transactions_number:{:?}",&transactions_vec.len());
                let mut index = transactions_vec.len();
                if index != 0 {
                    index = index - 1;
                    //判断是否是tdg
                    if transactions_vec[index.clone()].signer.is_empty() == true {
                        let raw_bytes_tdg = transactions_vec[index.clone()].tx_hash.clone();
                        info!("got tdg from sp:{:?}",raw_bytes_tdg.clone().len());
                        self.raw_bytes_tdg = raw_bytes_tdg.clone();
                        transactions_vec.remove(index);
                    }
                }
                let repeated_transactions = RepeatedField::from_vec(transactions_vec);
                block_body.set_transactions(repeated_transactions);
                block.set_body(block_body);
                let open_block = OpenBlock::from(block);
                info!("SignedProposal的blk:{:?}",&open_block.number());
                //qqf-code-end
            ...
            routing_key!(Consensus >> BlockWithProof) => {
                ...
                let mut bwf = proofed.clone();
                let mut block = bwf.take_blk().clone();
                let mut block_body = block.take_body().clone();
                let repeated_transactions = block_body.take_transactions().clone();
                let mut transactions_vec = repeated_transactions.into_vec();
                let mut index = transactions_vec.len();
                if index != 0 {
                    ...
                    if transactions_vec[index.clone()].signer.is_empty() == true {
                        let raw_bytes_tdg = transactions_vec[index.clone()].tx_hash.clone();
                        info!("got tdg from bwf:{:?}",raw_bytes_tdg.clone().len());
                        self.raw_bytes_tdg = raw_bytes_tdg.clone();
                        transactions_vec.remove(index);
                    }
                }
                ...
                block.set_body(block_body);
                bwf.set_blk(block);
                //info!("proofed(before):{:?}",proofed.clone());
                //info!("bwf(now):{:?}",bwf.clone());
                let open_block = OpenBlock::from(bwf.take_blk());
                //qqf-code-end
                //let open_block = OpenBlock::from(proofed.take_blk());
                //qqf-code
                /*
                let mut fake_open_block = open_block.clone();
                let mut open_block_body = fake_open_block.body.clone();
                let mut vec = open_block_body.transactions.clone();
                if vec.is_empty() =! true{
                    let mut s = vec[0].clone();
                    let mut un_s = &mut s.transaction;
                    let mut tran = &mut un_s.unsigned;
                    tran.tx
                }
                */
                //qqf-code-end
    ...
    fn grow_up(&mut self) {
        ...
        match self.backlogs.complete(next_height) {
            Ok(closed_block) => {
                info!("postman notice executor to grow up to {}", next_height);
    ...
    fn execute_next_block(&mut self) {
        ...
        match self.backlogs.ready(next_height) {
            Ok(open_block) => {
                info!("postman send {}-th block to executor", next_height);
    ...
    //qqf-code
    /*
    pub fn execute_fake_block(&mut self, open_block: OpenBlock){
        self.fake_fsm_req_sender.send(open_block.clone());
    }
    */
    //qqf-code-end
