...
impl<Cost: CostType> Interpreter<Cost> {
    ...
    fn exec_instruction(
        ...
        match instruction {
            ...
            //读取key数据（可以从一致性读写集中获取）
            instructions::SLOAD => {
                let key = H256::from(&stack.pop_back());
                let word = U256::from(&*ext.storage_at_mut(&key)?);
                //let word = U256::from(*ext.storage_at_mut(&key)?);
                stack.push(word);
            }
            instructions::SSTORE => {
                ...
                let current_val = U256::from(&*ext.storage_at_mut(&address)?);
                //let current_val = U256::from(*ext.storage_at_mut(&address)?);
                ...
                ext.set_temp_storage_change(address.clone(), H256::from(&val).clone());
