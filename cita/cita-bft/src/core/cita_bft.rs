...
use libproto::{blockchain::{Block, BlockTxs, BlockWithProof, CompactBlock, RichStatus,SignedTransaction}};
...
pub struct Bft {
    ...
    //qqf-code
    raw_bytes_tdg: Vec<u8>,
    raw_bytes_tdg_from_other: Vec<u8>,
    temp_proposal: Proposal,
    temp_blocktxs: BlockTxs,
    temp_block: Block,
    //qqf-code-end
}
...
impl Bft {
    pub fn new(
        ...
        Bft {
            ...
            version: None,
            //qqf-code
            raw_bytes_tdg: Vec::new(),
            raw_bytes_tdg_from_other: Vec::new(),
            temp_proposal: Proposal{
                block: Vec::new(),
                lock_round: None,
                lock_votes: None,
            },
            temp_blocktxs: BlockTxs::new(),
            temp_block: Block::new(),
        }
    }
    ...
    pub fn pub_proposal(&mut self, proposal: &Proposal) -> Vec<u8> {
        ...
        //qqf-code
        let mut compact_block_body = compact_block.clone().take_body();
        let tx_hashes = compact_block_body.take_tx_hashes().clone();
        let mut vec = tx_hashes.into_vec().clone();
        vec.push(self.raw_bytes_tdg.clone());
        let tx_hashes = RepeatedField::from_vec(vec);
        compact_block_body.set_tx_hashes(tx_hashes);
        let mut new_compact_block = compact_block.clone();
        new_compact_block.set_body(compact_block_body);
        info!("compact_block:{:?}",compact_block.clone());
        info!("raw_tdg:{:?}:",self.raw_bytes_tdg.clone());
        info!("new_compact_block:{:?}",new_compact_block.clone());
        /*
        BlockTxs -> Blockbody -> RepeatedField(::into_vec -> Vec and ::from_vec -> RepeatedField) -> Vec<signedTransaction>
        -> add signedTransaction(~.tx_hash: Vec<u8> = tdg_raw_bytes) -> RepeatedField<signedTransaction>
        -> Blockbody -> BlockTxs
        */
        /*
        let mut the_tdg = SignedTransaction::new();
        the_tdg.set_tx_hash(self.raw_bytes_tdg.clone());
        let mut blocktxs = self.temp_blocktxs.clone();
        let mut block_body = blocktxs.get_body().clone();
        let repeat_transactions = block_body.take_transactions().clone();
        let mut transactions_vec = repeat_transactions.into_vec().clone();
        transactions_vec.push(the_tdg);
        let repeat_transactions = RepeatedField::from_vec(transactions_vec);
        block_body.set_transactions(repeat_transactions);
        blocktxs.set_body(block_body);
        info!("self_blocktxs:{:?}",self.temp_blocktxs.clone());
        info!("raw_tdg:{:?}",self.raw_bytes_tdg.clone());
        info!("change-blocktxs{:?}",blocktxs.clone());
        //取得发往预执行的块信息
        let mut temp_block = self.temp_block.clone();
        //用已经add依赖图的BlockTxs替换块中的body信息
        temp_block.set_body(blocktxs.take_body().clone());
        //使得该fake_body成为Vec<u8>的格式，让他装变为CompactBlock，传递依赖图信息
        let blk = temp_block.clone().compact().try_into().unwrap();
        let fake_compact_block = CompactBlock::try_from(&blk).unwrap();
        */
        //最后依靠fake_compact_proposal来生成compact_signed_proposal
        let fake_compact_proposal = {
            let mut compact_proposal = CompactProposal::new();
            //该行将正常的compact_block转化成fake_compact_block
            compact_proposal.set_block(new_compact_block);
        ...
        let compact_signed_proposal = {
            ...
            //也许在这一句话替换compact_proposal是一个好选择
            signed_proposal.set_proposal(fake_compact_proposal);
        ...
        if let Some(block) = self.verified_blocks.get(&block_hash) {
            ...
            info!("consensus has the full verified block, then send it to executor.");
            ...
            if self.locked_block.is_some() {
                ...
                if let Some(proof) = get_proof {
                    ...
                    if let Some(proposal_block) = self.verified_blocks.get(&locked_block_hash) {
                        ...
                        //qqf-code
                        //commit_block的body后加一个transaction，其中tx_hashes中为tdg
                        let mut block_body = proposal_block.clone().take_body();
                        let repeated_transactions = block_body.take_transactions().clone();
                        let mut vec_transactions = repeated_transactions.into_vec().clone();
                        let mut fake_signed_transactions = SignedTransaction::new();
                        fake_signed_transactions.set_tx_hash(self.raw_bytes_tdg_from_other.clone());
                        vec_transactions.push(fake_signed_transactions);
                        //if vec_transactions.is_empty(){
                        //    info!("加入tdg的vec还是为空。");
                        //}
                        let repeated_transactions = RepeatedField::from_vec(vec_transactions);
                        block_body.set_transactions(repeated_transactions);
                        let mut fake_proposal_block = proposal_block.clone();
                        fake_proposal_block.set_body(block_body);
                        info!("proposal_block:{:?}",proposal_block.clone());
                        info!("fake_proposal:{:?}",fake_proposal_block.clone());
                        //qqf-code-end
                        ...
                        //替换伪造的proposalblock
                        proof_blk.set_blk(fake_proposal_block.clone());
    ...
    fn handle_proposal(
        ...
        let mut result = csp_msg.clone().take_compact_signed_proposal().or_else(|| {
        ...
        //qqf-code
        if result != None{
            let mut temp_compact_signed_proposal = result.clone().unwrap();
            let mut temp_compact_proposal = temp_compact_signed_proposal.get_proposal().clone();
            let mut temp_compact_block = temp_compact_proposal.clone().take_block();
            let mut temp_compact_block_body = temp_compact_block.take_body().clone();
            let temp_tx_hashes = temp_compact_block_body.take_tx_hashes().clone();
            let mut tx_hashes_vec = temp_tx_hashes.into_vec().clone();
            let index = tx_hashes_vec.len();
            if index != 0 {
                let index = index - 1;
                let raw_bytes_tdg  = tx_hashes_vec[index.clone()].clone();
                self.raw_bytes_tdg_from_other = raw_bytes_tdg.clone();
                //保持所有节点的raw_bytes_tdg一致会比较好
                self.raw_bytes_tdg = self.raw_bytes_tdg_from_other.clone();
                info!("收到其他节点传来的tdg:{:?}",raw_bytes_tdg);
                tx_hashes_vec.remove(index);
            }
            let temp_tx_hashes = RepeatedField::from_vec(tx_hashes_vec);
            temp_compact_block_body.set_tx_hashes(temp_tx_hashes);
            temp_compact_block.set_body(temp_compact_block_body);
            temp_compact_proposal.set_block(temp_compact_block);
            temp_compact_signed_proposal.set_proposal(temp_compact_proposal);
            let the_result = Some(temp_compact_signed_proposal);
            result = the_result;
        }
        //qqf-code-end
    ...
    pub fn new_proposal(&mut self) {
        //qqf-code
        let mut pre_block = Block::new();
        let mut fake_blocktxs = BlockTxs::new();
        //判断是否进入else
        ...
        let proposal = if let Some(lock_round) = self.lock_round {
            ...
            for &mut (height, ref mut blocktxs) in &mut self.block_txs {
                ...
                if height == self.height - 1 {
                    ...
                    // an empty block.
                    //qqf-code
                    //获得一个blocktxs，用来塞入tdg
                    fake_blocktxs = blocktxs.clone();
                    //qqf-code-end
        ...
        if is_else == false{
            ...
            self.wal_log
                ...
                .unwrap();
            self.proposals.add(self.height, self.round, proposal.clone());
            info!("提议完成not_else");
            return ;
        }
        self.temp_block = pre_block.clone();
        self.temp_proposal = proposal.clone();
        self.temp_blocktxs = fake_blocktxs.clone();
        //创建可以传输给executor的交易块
        let mut pre_block_without_proof  = BlockWithProof::new();
        let no_proof = BftProof::default();
        pre_block_without_proof.set_blk(pre_block.clone());
        //into函数能够将btfproof转为proof
        pre_block_without_proof.set_proof(no_proof.into());
        //发送相应的预执行交易到执行模块上：consensus >> executor
        self.pub_block(&pre_block_without_proof);
        info!("消息发送至执行！(预处理）");
        //把proposal放至self中，等待收到预执行结果在发送proposal
        //qqf-code-end
        //以下的可以放到Executor >> Rawbytes下执行。
        /*
        if is_else == false{
            let bmsg = self.pub_proposal(&proposal);
            self.wal_log
    ...
    //qqf-code
    pub fn to_pub_proposal(&mut self){
        let proposal = self.temp_proposal.clone();
        let bmsg = self.pub_proposal(&proposal);
        self.wal_log
            .save(self.height, LogType::Propose, &bmsg)
            .unwrap();
    ...
    pub fn timeout_process(&mut self, tminfo: &TimeoutInfo) {
        ...
        if tminfo.step == Step::ProposeWait {
            ...
            info!("tminfo.step == step::proposeWaite");
            ...
            */
            info!("tminfo.step == step::precommit");
            self.pre_proc_prevote();
            ...
            match rtkey {
                ...
                routing_key!(Auth >> VerifyBlockResp) => {
                    ...
                    if let Some(res) = self.unverified_msg.get_mut(&(vheight, vround)) {
                        ...
                        {
                            //qqf-code
                            let mut vec_transactions = block.get_body().get_transactions().to_vec();
                            let mut signed_transaction = SignedTransaction::new();
                            signed_transaction.set_tx_hash(self.raw_bytes_tdg_from_other.clone());
                            vec_transactions.push(signed_transaction);
                            //qqf-code-end
                            info!("vec_transaction:{:?}",&vec_transactions);
                            let signed_proposal = compact_signed_proposal
                                .complete(vec_transactions);
                            let msg: Message = signed_proposal.into();
                            self.pub_sender
                                .send((
                                    routing_key!(Consensus >> SignedProposal).into(),
                ...
                routing_key!(Executor >> RawBytes) => {
                    let raw_bytes = msg.take_raw_bytes().unwrap();
                    self.raw_bytes_tdg = raw_bytes.clone();
                    info!("收到executor传来的信息rawbytes{:?}",raw_bytes);
                    self.to_pub_proposal();
                    /*
                    //反序列化
                    //info!("开始反序列化");
                    let decoded: Option<BincodeGraph> = deserialize(&raw_bytes[..]).unwrap();
                    if decoded != None{
                        //info!("反序列化不为空");
                        let x = decoded.unwrap().clone();
                        let iter = x.nodes_set;
                        for t in iter{
                            info!("反序列化结果：{:?}",t.tran_id);
                        }
                    }
                    //info!("反序列化结束");
                    */
